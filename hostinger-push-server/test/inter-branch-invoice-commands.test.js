...
function seed({includeCounter = true} = {}) {
  return {
    ...
    brands: {
      "brand-a": {
        id: "brand-a", name: "Brand A", main_branch_id: "main-a",
      },
      "brand-b": {
        id: "brand-b", name: "Brand B", main_branch_id: "main-b",
      },
    },
    product_groups: {
      "group-a": {
...
test("a missing Transfer counter follows Collection's lazy branch-local zero start", async () => {
  ...
  await withServer(firestore, async (baseUrl) => {
    ...
    assert.equal(response.status, 201);
    assert.equal((await response.json()).invoice_number, "AA000");
  });
  assert.deepEqual(firestore.document("inter_branch_invoice_counters", "branch-s"), {
    branch_id: "branch-s",
    branch_code: "AA",
    next_number: 1,
    last_invoice_number: "AA000",
    last_updated: fixedNow,
  });
});

test("Transfer and Collection counters use separate branch-local namespaces", async () => {
  const data = seed({includeCounter: false});
  data.branch_transaction_counters = {
    "branch-s": {branch_id: "branch-s", branch_code: "AA", next_number: 27},
  };
  const firestore = new FakeFirestore(data);
  await withServer(firestore, async (baseUrl) => {
    const response = await post(baseUrl, "/v1/inter-branch-invoices", {
      uid: "manager-s",
      key: "separate-counter-namespaces-0001",
      body: {
        receiving_branch_id: "branch-r",
        items: [{product_id: "product-a", unit_id: "primary", supplied_quantity: 1}],
      },
  ...
  assert.equal(
      firestore.document("branch_transaction_counters", "branch-s").next_number,
      27,
  );
  assert.equal(firestore.document("inter_branch_invoice_counters", "branch-s").next_number, 1);
});

test("Transfer accepts the exact existing source branch code without brand metadata", async () => {
  const data = seed({includeCounter: false});
  data.users["manager-s3"] = {
    name: "Third source manager", role: "manager", branchId: "branch-s3", isActive: true,
    mustChangePassword: false,
  };
  data.branches["branch-s3"] = {
    id: "branch-s3", name: "Third source", brand_id: "brand-a", branch_code: "TTH",
    branch_manager_id: "manager-s3",
  };
  const firestore = new FakeFirestore(data);
  await withServer(firestore, async (baseUrl) => {
    const response = await post(baseUrl, "/v1/inter-branch-invoices", {
      uid: "manager-s3",
      key: "source-branch-code-0001",
      body: {
        receiving_branch_id: "branch-r",
        items: [{product_id: "product-a", unit_id: "primary", supplied_quantity: 1}],
      },
    ...
    assert.equal(response.status, 201);
    assert.equal((await response.json()).invoice_number, "TTH000");
  });
});

test("source branches of the same brand keep independent Transfer sequences", async () => {
  const data = seed({includeCounter: false});
  ...
  await withServer(firestore, async (baseUrl) => {
    ...
    const first = await post(baseUrl, "/v1/inter-branch-invoices", {
      uid: "manager-s", key: "branch-a-0001", body,
    });
    const second = await post(baseUrl, "/v1/inter-branch-invoices", {
      uid: "manager-s2", key: "branch-b-0001", body,
    });
    assert.equal((await first.json()).invoice_number, "AA000");
    assert.equal((await second.json()).invoice_number, "ZZ000");
  });
  assert.equal(firestore.document("inter_branch_invoices", "historical").invoice_number, "AA0007");
  assert.equal(firestore.document("inter_branch_invoice_counters", "branch-s").next_number, 1);
  assert.equal(firestore.document("inter_branch_invoice_counters", "branch-s2").next_number, 1);
});

test("existing Transfer counters continue and stop safely after 999", async () => {
  const data = seed();
  data.inter_branch_invoice_counters["branch-s"].next_number = 999;
  data.inter_branch_invoices = {
    historical: {id: "historical", invoice_number: "AA0007"},
  };
  const firestore = new FakeFirestore(data);
  await withServer(firestore, async (baseUrl) => {
    ...
    const finalNumber = await post(baseUrl, "/v1/inter-branch-invoices", {
      uid: "manager-s", key: "branch-final-0999", body,
    });
    assert.equal((await finalNumber.json()).invoice_number, "AA999");
    const overflow = await post(baseUrl, "/v1/inter-branch-invoices", {
      uid: "manager-s", key: "branch-overflow-1000", body,
    });
    assert.equal(overflow.status, 409);
    assert.equal((await overflow.json()).error.code, "counter-exhausted");
  });
  assert.equal(firestore.document("inter_branch_invoices", "historical").invoice_number, "AA0007");
});
...
test("main branches remain collector destinations and may receive cross-brand transfers", async () => {
  ...
  await withServer(firestore, async (baseUrl) => {
    ...
    assert.equal(created.invoice_number, "AA001");
...
test("invalid Transfer counters fail without allocating an invoice", async () => {
  const data = seed();
  data.inter_branch_invoice_counters["branch-s"].next_number = 1001;
  const firestore = new FakeFirestore(data);
  await withServer(firestore, async (baseUrl) => {
    const response = await post(baseUrl, "/v1/inter-branch-invoices", {
      uid: "manager-s",
  ...
  assert.equal(firestore.documents("inter_branch_invoices").length, 0);
  assert.equal(firestore.document("inter_branch_invoice_counters", "branch-s").next_number, 1001);
});
...
test("cross-brand normal destinations use the receiving catalog in both directions", async () => {
  ...
  await withServer(firestore, async (baseUrl) => {
    ...
    assert.equal(aToB.invoice_number, "AA001");
    assert.equal(bToA.invoice_number, "CC001");
